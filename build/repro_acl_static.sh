...
#        do
            ...
            echo ./nnt -f 2 -a -i $it -t $TPLS -m $modelname -b 1 -l $lr -d $dc -H $h -K $DIMS $TRAIN $DEV > $modelname/trainer_log.txt
            ./nnt -f 2 -a -i $it -t $TPLS -m $modelname -b 1 -l $lr -d $dc -H $h -K $DIMS $TRAIN $DEV 2>> $modelname/trainer_log.txt
